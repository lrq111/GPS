...
class DAPORewardManager:
    ...
    def __call__(self, config, data: DataProto, return_dict=True):
        ...
        for i in range(len(data)):
            ...
            collect_error_info = config.get("self_reflection", {}).get("enabled", True)
            
                                               
            if extra_info is None:
                extra_info = {}
            extra_info_with_collect = extra_info.copy()
            extra_info_with_collect["collect_error_info"] = collect_error_info
            
            result = self.compute_score(
                data_source=data_source,
                solution_str=response_str,
                ground_truth=ground_truth,
                extra_info=extra_info_with_collect,
            )
            ...
            if error_info is not None:
                if "error_info_list" not in reward_extra_info:
                    reward_extra_info["error_info_list"] = []
                reward_extra_info["error_info_list"].append(error_info)
            else:
                if "error_info_list" not in reward_extra_info:
                    reward_extra_info["error_info_list"] = []
                reward_extra_info["error_info_list"].append(None)
