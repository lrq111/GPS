...
def default_compute_score(data_source, solution_str, ground_truth, extra_info=None, sandbox_fusion_url=None, concurrent_semaphore=None):
    ...
    elif data_source in ["condqa", "sharcqa"]:
        from . import condqa
        # print("condqa compute reward function\n")
                                                           
        collect_error_info = extra_info.get("collect_error_info", False) if extra_info else False
        res = condqa.compute_score(solution_str, ground_truth, extra_info, collect_error_info=collect_error_info)
    else:
