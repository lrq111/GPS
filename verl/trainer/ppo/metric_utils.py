...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    metrics = {
        ...
        # correct, format, length
        "critic/correct_rewards/mean": np.mean(batch.non_tensor_batch["correct_score"]),
        "critic/correct_rewards/max": np.max(batch.non_tensor_batch["correct_score"]),
        "critic/correct_rewards/min": np.min(batch.non_tensor_batch["correct_score"]),
        "critic/format_rewards/mean": np.mean(batch.non_tensor_batch["format_score"]),
        "critic/format_rewards/max": np.max(batch.non_tensor_batch["format_score"]),
        "critic/format_rewards/min": np.min(batch.non_tensor_batch["format_score"]), 
        "critic/length_rewards/mean": np.mean(batch.non_tensor_batch["length_score"]),
        "critic/length_rewards/max": np.max(batch.non_tensor_batch["length_score"]),
        "critic/length_rewards/min": np.min(batch.non_tensor_batch["length_score"]), 
        "critic/ent_score/mean": np.mean(batch.non_tensor_batch["ent_score"]),
        "critic/ent_score/max": np.max(batch.non_tensor_batch["ent_score"]),
        "critic/ent_score/min": np.min(batch.non_tensor_batch["ent_score"]), 
        # "critic/rule_citation_score/mean": np.mean(batch.non_tensor_batch["rule_citation_score"]),
        # "critic/rule_citation_score/max": np.max(batch.non_tensor_batch["rule_citation_score"]),
        # "critic/rule_citation_score/min": np.min(batch.non_tensor_batch["rule_citation_score"]), 
        # "critic/dag_consistency_score/mean": np.mean(batch.non_tensor_batch["dag_consistency_score"]),
        # "critic/dag_consistency_score/max": np.max(batch.non_tensor_batch["dag_consistency_score"]),
        # "critic/dag_consistency_score/min": np.min(batch.non_tensor_batch["dag_consistency_score"]), 
        **(
            {
                # values
    ...
    for data_source, uid2var2vals in data_src2uid2var2vals.items():
        for uid, var2vals in uid2var2vals.items():
            for var_name, var_vals in var2vals.items():
                first_val = var_vals[0]
                if isinstance(first_val, str) or not isinstance(first_val, numbers.Number):
                    continue
