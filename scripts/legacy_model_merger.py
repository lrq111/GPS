...
class BaseModelMerger(ABC):
    def __init__(self, config: ModelMergerConfig):
        ...
        # Check if config.json exists in the specified path, if not, try huggingface subdirectory
        original_path = self.hf_model_config_path
        config_json_path = os.path.join(self.hf_model_config_path, "config.json")
        huggingface_config_path = os.path.join(self.hf_model_config_path, "huggingface", "config.json")
        
        if not os.path.exists(config_json_path) and os.path.exists(huggingface_config_path):
            self.hf_model_config_path = os.path.join(self.hf_model_config_path, "huggingface")
            print(f"Config.json not found in {original_path}, using {self.hf_model_config_path} instead")
